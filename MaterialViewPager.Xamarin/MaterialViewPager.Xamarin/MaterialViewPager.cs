...
namespace MaterialViewPager
{
    ...
    public MaterialViewPager(Context context) : base(context) {
        
    }

    public MaterialViewPager(Context context, IAttributeSet attrs) : base(context, attrs) {
        Settings.HandleAttributes(context, attrs);
    }

    public MaterialViewPager(Context context, IAttributeSet attrs, int defStyleAttr) : base(context, attrs, defStyleAttr) {
        Settings.HandleAttributes(context, attrs);
    }

    [TargetApi(Value = (int)BuildVersionCodes.Lollipop)]
    public MaterialViewPager(Context context, IAttributeSet attrs, int defStyleAttr, int defStyleRes) : base(context, attrs, defStyleAttr, defStyleRes) {
        Settings.HandleAttributes(context, attrs);
    }
...

        {
            ...
            if (headerId == -1) {
                if (Settings.AnimatedHeaderImage)
                    headerId = Resource.Layout.material_view_pager_moving_header;
                else
                    headerId = Resource.Layout.material_view_pager_imageview_header;
            }
